...
;(function() {
    ...
    jsPlumbUtil = {
            ...
            for (var i in b) {
                if (c[i] == null || this.isString(b[i]) || this.isBoolean(b[i]))
                    ...
                    if (this.isArray(b[i])/* && this.isArray(c[i]) */) {
                        var ar = [];
                        // if c's object is also an array we can keep its
						// values.
                        ...
                        // overwite c's value with an object if it is not
						// already one.
        ...
        //
        // chain a list of functions, supplied by [ object, method name, args ],
		// and return on the first
        // one that returns the failValue. if none return the failValue, return
		// the successValue.
        //
        ...
        populate : function(model, values) {		
            // for a string, see if it has parameter matches, and if so, try to
        ...
        /**
		 * calculates a perpendicular to the line fromPoint->toPoint, that
		 * passes through toPoint and is 'length' long.
		 * 
		 * @param fromPoint
		 * @param toPoint
		 * @param length
        ...
        /**
		 * EventGenerator Superclass for objects that generate events - jsPlumb
		 * extends this, as does jsPlumbUIComponent, which all the UI elements
		 * extend.
		 */
        EventGenerator : function() {
            var _listeners = {}, self = this, eventsSuspended = false;
            
            // this is a list of events that should re-throw any errors that
			// occur during their dispatch. as of 1.3.0 this is private to
            // jsPlumb, but it seems feasible that people might want to
			// manipulate this list. the thinking is that we don't want event
            // listeners to bring down jsPlumb - or do we. i can't make up my
			// mind about this, but i know i want to hear about it if the
			// "ready"
            // event fails, because then my page has most likely not
			// initialised. so i have this halfway-house solution. it will be
			// interesting
			 ...
			 * 
			 * Parameters: event - name of the event to bind to. listener -
			 * function to execute.
			 */
			 ...
			 * 
			 * Parameters: event - event to fire value - value to pass to the
			 * event listener(s). originalEvent - the original event from the
			 * browser
			 */			
            this.fire = function(event, value, originalEvent) {
                if (!eventsSuspended && _listeners[event]) {
                    for ( var i = 0; i < _listeners[event].length; i++) {
                        // doing it this way rather than catching and then
						// possibly re-throwing means that an error propagated
						// by this
                        // method will have the whole call stack available in
						// the debugger.
                        if (jsPlumbUtil.findWithFunction(eventsToDieOn, function(e) { return e === event}) != -1)
                            _listeners[event][i](value, originalEvent);
                        else {
                            // for events we don't want to die on, catch and
							// log.
                            try {
                                _listeners[event][i](value, originalEvent);
                            } catch (e) {
			 ...
			 * event.
			 * 
			 * Parameters: event - optional. constrains the clear to just
		 ...
		 * either SVG, Canvas elements, or VML.
		 * 
		 * This file contains the base functionality for DOM type adapters.
		 * 
		 * Copyright (c) 2010 - 2013 Simon Porritt (http://jsplumb.org)
		 * 
		 * http://jsplumb.org http://github.com/sporritt/jsplumb
		 * http://code.google.com/p/jsplumb
		 * 
		 * Dual licensed under the MIT and GPL2 licenses.
		 */
;(function() {
	...
	var DragManager = function(_currentInstance) {		
        ...
        /**
		 * register some element as draggable. right now the drag init stuff is
		 * done elsewhere, and it is possible that will continue to be the case.
		 */
		this.register = function(el) {
			...
			// look for child elements that have endpoints and register them
			// against this draggable.
		...
		/**
		 * notification that an endpoint was added to the given el. we go up
		 * from that el's parent node, looking for a parent that has been
		 * registered as a draggable. if we find one, we add this el to that
		 * parent's list of elements to update on drag (if it is not there
		 * already)
		 */
    ...
    // for those browsers that dont have it. they still don't have it! but at
...
;(function() {
	...
	var _att = function(el, attName) { return jsPlumb.CurrentLibrary.getAttribute(_gel(el), attName); },
		...
		/*
		 * Class:jsPlumbUIComponent Abstract superclass for UI components
		 * Endpoint and Connection. Provides the abstraction of
		 * paintStyle/hoverPaintStyle, and also extends
		 * jsPlumbUtil.EventGenerator to provide the bind and fire methods.
		 */
		jsPlumbUIComponent = window.jsPlumbUIComponent = function(params) {
			...
			// TODO issue 116 showed a problem with this - it seems 'a' that is
			// in
			// the clone function's scope is shared by all invocations of it,
			// the classic
			// JS closure problem. for now, jsPlumb does a version of this
			// inline where
			// it used to call clone. but it would be nice to find some time to
			// look
			// further at this.
			this.clone = function() {
				var o = new Object();
			...
			// user can supply a beforeDetach callback, which will be executed
			// before a detach
			...
			// user can supply a beforeDrop callback, which will be executed
			// before a dropped
			// connection is confirmed. user can return false to reject
			// connection.
			var beforeDrop = params.beforeDrop;
this.isDropAllowed = function(sourceId, targetId, sourceName,
				targetName, scope, connection, dropEndpoint, name, id, flag, done,
				doPreAction, postAction, packageName, chargeMode, processName,disactivate_SERVICE,add_TO_CRBT_BLACKLIST,resume_FROM_CRBT_BLACKLIST) {
			var r = self._jsPlumb.checkCondition("beforeDrop", {
				...
				postActionPackageName : packageName,
				postActionChargeMode : chargeMode,
				postActionProcessName : processName,
				postActionDisactivate_SERVICE : disactivate_SERVICE,
				postActionAdd_TO_CRBT_BLACKLIST : add_TO_CRBT_BLACKLIST,
				posActionResume_FROM_CRBT_BLACKLIST : resume_FROM_CRBT_BLACKLIST
			});
			if (beforeDrop) {
				try {
					r = beforeDrop({
			...
			// helper method to update the hover style whenever it, or
			// paintStyle, changes.
			// we use paintStyle as the foundation and merge hoverPaintStyle
			// over the
			// top.
			var _updateHoverStyle = function() {
				if (paintStyle && hoverPaintStyle) {
					...
					// we want the fillStyle of paintStyle to override a
			 ...
			 * 
			 * Returns: the component's paint style. if there is no
			 * hoverPaintStyle set then this will be the paint style used all
			 * the time, otherwise this is the style used when the mouse is not
			 * hovering.
			 */
		    ...
		    /*
			 * Sets the paint style to use when the mouse is hovering over the
			 * element. This is null by default. The hover paint style is
			 * applied as extensions to the paintStyle; it does not entirely
			 * replace it. This is because people will most likely want to
			 * change just one thing when hovering, say the color for example,
			 * but leave the rest of the appearance the same.
			 * 
			 * Parameters: style - Style to use when the mouse is hovering.
			 * doNotRepaint - if true, the component will not be repainted.
			 * useful when setting things up initially.
			 */
			 ...
			 * 
			 * Returns: the component's hover paint style. may be null.
			 */
			 ...
			 * 
			 * Parameters: hover - hover state boolean ignoreAttachedElements -
			 * if true, does not notify any attached elements of the change in
			 * hover state. used mostly to avoid infinite loops.
			 */
		    this.setHover = function(hover, ignoreAttachedElements, timestamp) {
		    	// while dragging, we ignore these events. this keeps the UI
				// from flashing and
				...
				if (!self._jsPlumb.currentlyDragging && !self._jsPlumb.isHoverSuspended()) {
					...
					// get the list of other affected elements, if supported by
					// this component.
					// for a connection, its the endpoints. for an endpoint, its
					// the connections! surprise.
		    ...
		    var _updateAttachedElements = function(state, timestamp, sourceElement) {
		    	var affectedElements = self.getAttachedElements();		// implemented
																		// in
																		// subclasses
		    	if (affectedElements) {
		    		for (var i = 0, j = affectedElements.length; i < j; i++) {
		    			if (!sourceElement || sourceElement != affectedElements[i])
		    				affectedElements[i].setHover(state, true, timestamp);			// tell
																							// the
																							// attached
																							// elements
																							// not
			...
			/*
			 * Function: setType Sets the type, removing all existing types.
			 */
			...
			/*
			 * Function : getType Gets the 'types' of this component.
			 */
			...
			/**
			 * Function: reapplyTypes Reapply all existing types, but with the
			 * given new params.
			 */
			...
			/*
			 * Function: addType adds a type. will not be re-added it already
			 * exists.
			 */
		...
		overlayCapableJsPlumbUIComponent = window.overlayCapableJsPlumbUIComponent = function(params) {
			...
			var processOverlay = function(o) {
				...
				if (_isArray(o)) {	// this is for the shorthand ["Arrow", {
									// width:50 }] syntax
					// there's also a three arg version:
					// ["Arrow", { width:50 }, {location:0.7}]
					// which merges the 3rd arg into the 2nd.
					var type = o[0],
						// make a copy of the object so as not to mess up anyone
						// else's reference...
    ...
    var log = null,
        ...
        canvasList = [],
        sizes = [],
        // listeners = {}, // a map: keys are event types, values are lists of
		// listeners.
		...
		/**
		 * appends an element to some other element, which is calculated as
		 * follows:
		 * 
		 * 1. if _currentInstance.Defaults.Container exists, use that element.
		 * 2. if the 'parent' parameter exists, use that. 3. otherwise just use
		 * the root element (for DOM usage, the document body).
		 * 
		 */
		...
		/**
		 * YUI, for some reason, put the result of a Y.all call into an object
		 * that contains a '_nodes' array, instead of handing back an array-like
		 * object like the other libraries do.
		 */
		...
		/**
		 * Draws an endpoint and its connections. this is the main entry point
		 * into drawing connections as well as endpoints, since jsPlumb is
		 * endpoint-centric under the hood.
		 * 
		 * @param element
		...
		_draw = function(element, ui, timestamp, clearEdits) {
			...
			// TODO is it correct to filter by headless at this top level? how
			// would a headless adapter ever repaint?
		...
		/*
		 * prepares a final params object that can be passed to _newConnection,
		 * taking into account defaults, events, etc.
		 */
		_prepareConnectionParams = function(params, referenceParams) {
			...
			// hotwire endpoints passed as source or target to
			// sourceEndpoint/targetEndpoint, respectively.
			...
			// at this point, if we have source or target Endpoints, they were
			// not new and we should mark the
			// flag to reflect that. this is for use later with the
			// deleteEndpointsOnDetach flag.
			...
			// if source endpoint mandates connection type and nothing specified
			// in our params, use it.
			if (!_p.type && _p.sourceEndpoint)
				_p.type = _p.sourceEndpoint.connectionType;
			
			// copy in any connectorOverlays that were specified on the source
			// endpoint.
			// it doesnt copy target endpoint overlays. i'm not sure if we want
			// it to or not.
			...
			// if there's a target specified (which of course there should be),
			// and there is no
			// target endpoint specified, and 'newConnection' was not set to
			// true, then we check to
			// see if a prior call to makeTarget has provided us with the specs
			// for the target endpoint, and
			// we use those if so. additionally, if the makeTarget call was
			// specified with 'uniqueEndpoint' set
			// to true, then if that target endpoint has already been created,
			// we re-use it.
		...
		/**
		 * adds the connection to the backing model, fires an event if necessary
		 * and then redraws
		 */
		_finaliseConnection = function(jpc, params, originalEvent) {
            ...
            // except that if jpc has a suspended endpoint it's not true to say
			// the
            // connection is new; it has just (possibly) moved. the question is
			// whether
            // to make that call here or in the anchor manager. i think perhaps
			// here.
			...
			if (!params.doNotFireConnectionEvent && params.fireEvent !== false) {
				...
				// this is from 1.3.11 onwards. "jsPlumbConnection" always felt
				// so unnecessary, so
				// I've added this alias in 1.3.11, with a view to removing
				// "jsPlumbConnection" completely in a future version. be aware,
				// of course, you should only register listeners for one or the
		...
		/**
		 * for the given endpoint params, returns an appropriate parent element
		 * for the UI elements that will be added. this function is used by
		 * _newEndpoint (directly below), and also in the makeSource function in
		 * jsPlumb.
		 * 
		 * the logic is to first look for a "container" member of params, and
		 * pass that back if found. otherwise we handoff to the 'getParent'
		 * function in the current library.
		 */
		...
		/**
		 * factory method to prepare a new endpoint. this should always be used
		 * instead of creating Endpoints manually, since this method attaches
		 * event listeners and an id.
		 */
		...
		_setVisible = function(el, state, alsoChangeEndpoints) {
			...
			_operation(id, function(jpc) {
				if (state && alsoChangeEndpoints) {		
					// this test is necessary because this functionality is new,
					// and i wanted to maintain backwards compatibility.
					// this block will only set a connection to be visible if
					// the other endpoint in the connection is also visible.
					var oidx = jpc.sourceId === id ? 1 : 0;
					if (jpc.endpoints[oidx].isVisible()) jpc.setVisible(true);
				}
				else  // the default behaviour for show, and what always
						// happens for hide, is to just set the visibility
						// without getting clever.
		 ...
		 * or if 'recalc' is true in order to force a recalculation, we get the
		 * current values.
		 */
		 ...
		 * 
		 * where "index in instance" is a monotonically increasing integer that
		 * starts at 0, for each instance. this method is used not only to
		 * assign ids to elements that do not have them but also to connections
		 * and endpoints.
		 */
		 ...
		 *            original function to wrap; may be null.
		 * @param newFunction
		 *            function to wrap the original with.
		 * @param returnOnThisValue
		 *            Optional. Indicates that the wrappedFunction should not be
		 *            executed if the newFunction returns a value matching
		 *            'returnOnThisValue'. note that this is a simple comparison
		 *            and only works for primitives right now.
		 */
        // TODO move to util.
...
// ---------------------------------------------------------
		...
		this.animate = function(el, properties, options) {
			...
			// onComplete repaints, just to make sure everything looks good at
		...
		/**
		 * checks for a listener for the given condition, executing it if found,
		 * passing in the given value. condition listeners would have been
		 * attached using "bind" (which is, you could argue, now overloaded,
		 * since firing click events etc is a bit different to what this does).
		 * i thought about adding a "bindCondition" or something, but decided
		 * against it, for the sake of simplicity. jsPlumb will never fire one
		 * of these condition events anyway.
		 */
		 ...
		 * the handler a 'proceed' function and a 'stop' function. The handler
		 * MUST execute one or other of these once it has made up its mind.
		 * 
		 * Note that although this reads the listener list for the given
		 * condition, it does not loop through and hit each listener, because
		 * that, with asynchronous callbacks, would be messy. so it uses only
		 * the first listener registered.
		 */ 
		...
		this.connect = function(params, referenceParams) {
			...
			// TODO probably a nicer return value if the connection was not
			// made. _prepareConnectionParams
			// will return null (and log something) if either endpoint was full.
			// what would be nicer is to
			// create a dedicated 'error' object.
			if (_p) {
				// a connect call will delete its created endpoints on detach,
				// unless otherwise specified.
				// this is because the endpoints belong to this connection only,
				// and are no use to
		...
		// delete every endpoint and their connections. distinct from reset
		// because we dont clear listeners here.
		...
		var fireDetachEvent = function(jpc, doFireEvent, originalEvent) {
			...
			if (doFireEvent) {
				...
				// introduced in 1.3.11..an alias because the original event
				// name is unwieldy. in future versions this will be the only
				// event and the other will no longer be fired.
		...
		this.detach = function() {
            ...
            var connType =  _currentInstance.Defaults.ConnectionType || _currentInstance.getDefaultConnectionType(),
				...
				if (conn) {             
					...
					var _p = jsPlumb.extend( {}, params); // a backwards
															// compatibility
															// hack: source
															// should be thought
															// of as 'params' in
		...
		// detach every connection but leave endpoints in place (unless a
		// connection is set to auto delete them)
		...
		this.selectEndpoints = function(params) {
			...
			for (var el in endpointsByElement) {
				...
				// if they requested 'either' then just match scope. otherwise
				// if they requested 'source' (not as a wildcard) then we have
				// to match only endpoints that have isSource set to to true,
				// and the same thing with isTarget.
		...
		// gets a library-agnostic selector. not necessary for use outside of
		// jsplumb, since
		...
		/**
		 * callback from the current library to tell us to prepare ourselves
		 * (attach mouse listeners etc; can't do that until the library has
		 * provided a bind method)
		 */
		this.init = function() {
			if (!initialized) {                
                _currentInstance.anchorManager = new jsPlumb.AnchorManager({jsPlumbInstance:_currentInstance});                
				_currentInstance.setRenderMode(_currentInstance.Defaults.RenderMode);  // calling
																						// the
																						// method
																						// forces
																						// the
		 ...
		 * 
		 * 
		 * Returns: The newly created Anchor. Throws: an error if a named anchor
		 * was not found.
		 */
		this.makeAnchor = function() {
			...
			if (specimen.compute && specimen.getOrientation) return specimen;  // TODO
																				// hazy
																				// here
																				// about
																				// whether
			...
			// is it the name of an anchor type?
				...
				// newAnchor =
				// jsPlumb.Anchors[arguments[0]]({elementId:elementId,
		...
		/**
		 * Makes a dynamic anchor from the given list of anchors (which may be
		 * in shorthand notation as strings or dimension arrays, or Anchor
		 * objects themselves) and the given, optional, anchorSelector function
		 * (jsPlumb uses a default if this is not provided; most people will not
		 * need to provide this - i think).
		 */
		...
		this.makeTarget = function(el, params, referenceParams) {						
			...
			_doOne = function(_el) {
				...
				// get the element's id and store the endpoint definition for
				// it. jsPlumb.connect calls will look for one of these,
				...
				_drop = function() {
					...
					// unlock the source anchor to allow it to refresh its
					...
					// var _continue = jpc.isDropAllowed(jpc.sourceId,
					// _getId(_el), jpc.scope);
					var _continue = proxyComponent.isDropAllowed(jpc.sourceId, _getId(_el), jpc.scope, jpc, null);		
					
					// regardless of whether the connection is ok, reconfigure
					// the existing connection to
					// point at the current info. we need this to be correct for
					// the detach event that will follow.
					// clear the source endpoint from the list to detach. we
					// will detach this connection at this
					// point, but we want to keep the source endpoint. the
					// target is a floating endpoint and should
					// be removed. TODO need to figure out whether this code can
					// result in endpoints kicking around
					// when they shouldnt be. like is this a full detach of a
					// connection? can it be?
					...
					// reinstate any suspended endpoint; this just puts the
					// connection back into
					// a state in which it will report sensible values if
					// someone asks it about
					// its target. we're going to throw this connection away
					// shortly so it doesnt matter
					...
					if (_continue) {
						...
						if (p.uniqueEndpoint) _targetEndpoints[elid] = newEndpoint;  // may
																						// of
																						// course
																						// just
																						// store
						...
						// if the anchor has a 'positionFinder' set, then
						// delegate to that function to find
						...
						if (newEndpoint.anchor.positionFinder != null) {
							...
							// now figure an orientation for it..kind of hard to
							// know what to do actually. probably the best thing
							// i can do is to
							// support specifying an orientation in the anchor's
							// spec. if one is not supplied then i will make the
							// orientation
							// be what will cause the most natural link to the
							// source: it will be pointing at the source, but it
							// needs to be
							// specified in one axis only, and so how to make
							// that choice? i think i will use whichever axis is
							// the one in which
						...
						var c = _currentInstance.connect({
							...
							// 'endpointWillMoveAfterConnection' is set by the
							// makeSource function, and it indicates that the
							// given endpoint will actually transfer from the
							// element it is currently attached to to some other
							// element after a connection has been established.
							// in that case, we do not want to fire the
							// connection event, since it will have the wrong
							// data in it; makeSource will do it for us.
							// this is controlled by the 'parent' parameter on a
							// makeSource call.
							doNotFireConnectionEvent:source.endpointWillMoveAfterConnection
						});

						// delete the original target endpoint. but only want to
						// do this if the endpoint was created
					...
					// if not allowed to drop...
						...
						// TODO this code is identical (pretty much) to what
						// happens when a connection
						// dragged from a normal endpoint is in this situation.
						// refactor.
						...
						if (jpc.suspendedEndpoint) {
							...
							if (jpc.isReattach()) {
								...
								source.detach(jpc, false, true, true, originalEvent);  // otherwise,
																						// detach
																						// the
																						// connection
																						// and
		...
		this.unmakeTarget = function(el, doNotClearArrays) {
			...
			// TODO this is not an exhaustive unmake of a target, since it does
			// not remove the droppable stuff from
			// the element. the effect will be to prevent it form behaving as a
			// target, but it's not completely purged.
			if (!doNotClearArrays) {
		...
		this.makeSource = function(el, params, referenceParams) {
			...
			var jpcl = jsPlumb.CurrentLibrary,
				...
				onMaxConnections = p.onMaxConnections,
				_doOne = function(_el) {
					// get the element's id and store the endpoint definition
					// for it. jsPlumb.connect calls will look for one of these,
					// and use the endpoint definition if found.
					var elid = _getId(_el),
					...
					// set scope if its not set in dragOptions but was passed in
					// in params
					...
					dragOptions[stopEvent] = _wrap(dragOptions[stopEvent], function() { 							
						...
						if (ep.connections.length == 0)
							...
							// provided. the one we were using to drag
							// the connection was just a placeholder that was
							// located at the place the user pressed the
					...
					// when the user presses the mouse, add an Endpoint, if we
					// are enabled.
					var mouseDownListener = function(e) {
	                    ...
	                    // if a filter was given, run it, and return if it says
						// no.
						if (p.filter) {
						...
						// if there is a parent, the endpoint will actually be
						// added to it now, rather than the div
						// that was the source. in that case, we have to adjust
						// the anchor position so it refers to
						// the parent.
						if (p.parent) {
						...
						// we need to override the anchor in here, and force
						// 'isSource', but we don't want to mess with
						// the params passed in, because after a connection is
						// established we're going to reset the endpoint
						...
						// if a parent was given we need to turn that into a
						// "container" argument. this is, by default,
						// the parent of the element we will move to, so parent
						// of p.parent in this case. however, if
						// the user has specified a 'container' on the endpoint
						// definition or on
						...
						// we set this to prevent connections from firing attach
						// events before this function has had a chance
	                    ...
	                    var _delTempEndpoint = function() {
							// this mouseup event is fired only if no dragging
							// occurred, by jquery and yui, but for mootools
							// it is fired even if dragging has occurred, in
							// which case we would blow away a perfectly
							// legitimate endpoint, were it not for this check.
							// the flag is set after adding an
							// endpoint and cleared in a drag listener we set in
							// the dragOptions above.
						...
						// and then trigger its mousedown event, which will kick
						// off a drag, which will start dragging
						// a new connection from this endpoint.
						jpcl.trigger(ep.canvas, "mousedown", e);
						
					};
	               
	                // register this on jsPlumb so that it can be cleared by a
					// reset.
	                _currentInstance.registerListener(_el, "mousedown", mouseDownListener);
	                _sourceTriggers[elid] = mouseDownListener;

	                // lastly, if a filter was provided, set it as a dragFilter
					// on the element,
	                // to prevent the element drag function from kicking in when
					// we want to
		...
		// sets the id of some element, changing whatever we need to to keep
		// track.
		...
		// called to notify us that an id WAS changed, and we should do our
		// changes, but we
		...
		var _suspendDrawing = false,
         ...
         // set whether or not drawing is suspended. you should use this when
			// doing bulk painting,
        ...
        // suspends drawing, runs the given function, then re-enables drawing
		// (and repaints,
		...
		/*
		 * Property: SVG Constant for use with the setRenderMode method
		 */
		 /*
			 * Property: VML Constant for use with the setRenderMode method
			 */
		/*
		 * Property: CANVAS Constant for use with the setRenderMode method
		 */
		this.SVG = "svg";
		...
		/*
		 * Function: setRenderMode Sets render mode: jsPlumb.SVG or jsPlumb.VML.
		 * jsPlumb will fall back to VML if it determines that what you asked
		 * for is not supported (and that VML is). If you asked for VML but the
		 * browser does not support it, jsPlumb uses SVG.
		 * 
		 * Parameters: mode - a string representing the mode. Use one of the
		 * jsPlumb render mode constants as discussed above.
		 * 
		 * Returns: The render mode that jsPlumb set, which of course may be
		 ...
		 * 
		 * Returns: The current jsPlumb instance
		 */
		...
		/*
		 * Helper method to wrap an existing function with one of your own. This
		 * is used by the various implementations to wrap event callbacks for
		 * drag/drop etc; it allows jsPlumb to be transparent in its handling of
		 * these things. If a user supplies their own event callback, for
		 * anything, it will always be called.
		 */
		this.wrap = _wrap;			
		this.addListener = this.bind;
		
        /*
		 * helper method to take an xy location and adjust it for the parent's
		 * offset and scroll.
		 */
		this.adjustForParentOffsetAndScroll = function(xy, el) {
			...
			if (offsetParent != null) {
				...
				// offsetParent.offsetLeft + offsetParent.scrollLeft;
				// lastReturnValue[1] = lastReturnValue[1] -
				// offsetParent.offsetTop + offsetParent.scrollTop;
				// but i think it ignores margins. my reasoning was that it's
				// quicker to not hand off to some underlying
				// library.
...
;(function() {	
	...
	jsPlumb.AnchorManager = function(params) {
		var _amEndpoints = {},
            ...
            floatingConnections = {},
            // TODO this functions uses a crude method of determining
			// orientation between two elements.
	       // 'diagonal' should be chosen when the angle of the line between
			// the two centers is around
	       // one of 45, 135, 225 and 315 degrees. maybe +- 15 degrees.
            ...
            calculateOrientation = function(sourceId, targetId, sd, td, sourceAnchor, targetAnchor) {
                ...
                var theta = Math.atan2((td.centery - sd.centery) , (td.centerx - sd.centerx)),
                    ...
                    possiblyTranslateEdges = function(edges) {
                        // this function checks to see if either anchor is
						// Continuous, and if so, runs the suggested edge
                        // through the anchor: Continuous anchors can say which
						// faces they support, and they get to choose
                        // whether a certain face is honoured, or, if not, which
						// face to replace it with. the behaviour when
                        // choosing an alternate face is to try for the opposite
						// face first, then the next one clockwise, and then
            ...
            currySort = function(reverseAngles) {
                return function(a,b) {
                    ...
                    if (reverseAngles) {
                        /*
						 * if (a[0][0] < b[0][0]) r = true; else r = a[0][1] >
						 * b[0][1];
						 */
                        r = a[0][0] < b[0][0];
                    }
            ...
            placeAnchors = function(elementId, _anchorLists) {		
                ...
                placeSomeAnchors = function(desc, elementDimensions, elementPosition, unsortedConnections, isHorizontal, otherMultiplier, orientation) {
                    if (unsortedConnections.length > 0) {
                        var sc = _sortHelper(unsortedConnections, edgeSortFunctions[desc]), // puts
																							// them
																							// in
																							// order
																							// based
                        ...
                        // takes a computed anchor position and adjusts it for
						// parent offset and scroll, then stores it.
        ...
        var removeEndpointFromAnchorLists = function(endpoint) {
            (function(list, eId) {
                if (list) {  // transient anchors dont get entries in this
								// list.
        ...
        // updates the given anchor list by either updating an existing anchor's
		// info, or adding it. this function
        // also removes the anchor from its previous list, if the edge it is on
		// has changed.
        // all connections found along the way (those that are connected to one
		// of the faces this function
        // operates on) are added to the connsToPaint list, as are their
		// endpoints. in this way we know to repaint
        // them wthout having to calculate anything else about them.
        var _updateAnchorList = function(lists, theta, order, conn, aBoolean, otherElId, idx, reverse, edgeId, elId, connsToPaint, endpointsToPaint) {        
            // first try to find the exact match, but keep track of the first
			// index of a matching element id along the way.s
            var exactIdx = -1,
            ...
            if (exactIdx != -1) {
                ...
                var insertIdx = reverse ? firstMatchingElIdx != -1 ? firstMatchingElIdx : 0 : listToAddTo.length; // of
																													// course
																													// we
																													// will
																													// get
		...
		this.redraw = function(elementId, ui, timestamp, offsetToUI, clearEdits) {
			...
			if (!jsPlumbInstance.isSuspendDrawing()) {
				...
				// offsetToUI are values that would have been calculated in the
				// dragManager when registering
				// an endpoint for an element that had a parent (somewhere in
				// the hierarchy) that had been
				...
				// actually, first we should compute the orientation of this
				// element to all other elements to which
				// this element is connected with a continuous anchor (whether
				// both ends of the connection have
	            ...
	            for (var i = 0; i < endpointConnections.length; i++) {
	                ...
	                if (sourceContinuous || targetContinuous) {
		                ...
		                if (targetId == sourceId && (sourceContinuous || targetContinuous)) {
		                    // here we may want to improve this by somehow
							// determining the face we'd like
						    // to put the connector on. ideally, when drawing,
							// the face should be calculated
						    // by determining which face is closest to the point
							// at which the mouse button
							// was released. for now, we're putting it on the
							// top face.
		                    _updateAnchorList(
		                    ...
		                    if (!o) {
		                        ...
		                        // this would be a performance enhancement, but
								// the computed angles need to be clamped to
		                        // the (-PI/2 -> PI/2) range in order for the
								// sorting to work properly.
		                    /*
							 * orientationCache[oKey2] = {
							 * orientation:o.orientation, a:[o.a[1], o.a[0]],
							 * theta:o.theta + Math.PI, theta2:o.theta2 +
							 * Math.PI };
							 */
		                    }
				...
				// place Endpoints whose anchors are continuous but have no
				// Connections
				...
				// now that continuous anchors have been placed, paint all the
				// endpoints for this element
	            // TODO performance: add the endpoint ids to a temp array, and
				// then when iterating in the next
	            // loop, check that we didn't just paint that endpoint. we can
				// probably shave off a few more milliseconds this way.
				for (var i = 0; i < ep.length; i++) {				
                    ep[i].paint( { timestamp : timestamp, offset : myOffset, dimensions : myOffset.s });
				}
	            // ... and any other endpoints we came across as a result of the
				// continuous anchors.
				...
				// paint all the standard and "dynamic connections", which are
				// connections whose other anchor is
				// static and therefore does need to be recomputed; we make sure
				// that happens only one time.
	
				// TODO we could have compiled a list of these in the first pass
				// through connections; might save some time.
				for (var i = 0; i < endpointConnections.length; i++) {
					...
					if (otherEndpoint.anchor.constructor == jsPlumb.DynamicAnchor) {			 							
						...
						// all the connections for the other endpoint now need
						// to be repainted
				...
				// paint current floating connection for this element, if there
				// is one.
        ...
        var ContinuousAnchor = function(anchorParams) {
            ...
            // if the given edge is suported, returns it. otherwise looks for a
			// substitute that _is_
    ...
    /**
	 * Anchors model a position on some element at which an Endpoint may be
	 * located. They began as a first class citizen of jsPlumb, ie. a user was
	 * required to create these themselves, but over time this has been replaced
	 * by the concept of referring to them either by name (eg. "TopMiddle"), or
	 * by an array describing their coordinates (eg. [ 0, 0.5, 0, -1 ], which is
	 * the same as "TopMiddle"). jsPlumb now handles all of the creation of
	 * Anchors without user intervention.
	 */
	 ...
	 * position relative to the anchor it is floating relative to. It is used
	 * when creating a connection through drag and drop.
	 * 
	 * TODO FloatingAnchor could totally be refactored to extend Anchor just
	 * slightly.
	 */
    jsPlumb.FloatingAnchor = function(params) {
        ...
        // set these to 0 each; they are used by certain types of connectors in
		// the loopback case,
        // when the connector is trying to clear the element it is on. but for
		// floating anchor it's not
        // very important.
        this.x = 0; this.y = 0;
        ...
        this.compute = function(params) {
            ...
            result = [ xy[0] + (size[0] / 2), xy[1] + (size[1] / 2) ]; // return
																		// origin
																		// of
																		// the
																		// element.
        ...
        this.getOrientation = function(_endpoint) {
            if (orientation) return orientation;
                ...
                // anchor: if it declares zero for some direction, we declare
				// zero too. this might not be the most awesome. perhaps we can
				// come
                // up with a better way. it's just so that the line we draw
				// looks like it makes sense. maybe this wont make sense.
		 ...
		 * over another anchor; we want to assume that anchor's orientation for
        ...
        /**
		 * notification the endpoint associated with this anchor is no longer
		 * hovering over another anchor; we should resume calculating
    ...
    /*
	 * A DynamicAnchor is an Anchor that contains a list of other Anchors, which
	 * it cycles through at compute time to find the one that is located closest
	 * to the center of the target element, and returns that Anchor's compute
	 * method result. this causes endpoints to follow each other with respect to
	 * the orientation of their target elements, which is a useful feature for
	 * some applications.
	 * 
	 */
    jsPlumb.DynamicAnchor = function(params) {
        ...
        var _curAnchor = _anchors.length > 0 ? _anchors[0] : null,
            ...
            // helper method to calculate the distance between the centers of
			// the two elements.
            ...
            // default method uses distance between element centers. you can
			// provide your own method in the dynamic anchor
            // constructor (and also to jsPlumb.makeDynamicAnchor). the
			// arguments to it are four arrays:
            // xy - xy loc of the anchor's element
            // wh - anchor's element's dimensions
            // txy - xy loc of the element of the other anchor in the connection
            // twh - dimensions of the element of the other anchor in the
			// connection.
        ...
        this.compute = function(params) {				
            ...
            if (self.locked || txy == null || twh == null)
                ...
                params.timestamp = null; // otherwise clear this, i think. we
											// want the anchor to compute.
...
// ------- position assign anchors -------------------
	...
	jsPlumb.Anchors["Assign"] = _curryAnchor(0, 0, 0, 0, "Assign", function(anchor, params) {
		// find what to use as the "position finder". the user may have supplied
		// a String which represents
		// the id of a position finder in jsPlumb.AnchorPositionFinders, or the
		// user may have supplied the
		// position finder as a function. we find out what to use and then set
		// it on the anchor.
		var pf = params.position || "Fixed";
		anchor.positionFinder = pf.constructor == String ? params.jsPlumbInstance.AnchorPositionFinders[pf] : pf;
		// always set the constructor params; the position finder might need
		// them later (the Grid one does,
		// for example)
		anchor.constructorParams = params;
	});	

    // these are the default anchor positions finders, which are used by the
	// makeTarget function. supplying
    // a position finder argument to that function allows you to specify where
	// the resulting anchor will
    // be located
...
// ------- perimeter anchors -------------------
    ...
    // creates a placeholder div for dragging purposes, adds it to the DOM, and
	// pre-computes its offset.
    ...
    var _makeFloatingEndpoint = function(paintStyle, referenceAnchor, endpoint, referenceCanvas, sourceElement, _jsPlumb, _newEndpoint) {			
        ...
        // setting the scope here should not be the way to fix that mootools
		// issue. it should be fixed by not
        // adding the floating endpoint as a droppable. that makes more sense
		// anyway!
...
// END TYPE
            ...
            if (!doNotChangeConnections) {
                for (var i = 0; i < self.connections.length; i++) {
                    ...
                    if (!doNotNotifyOtherEndpoint) {
                        ...
                        // only change the other endpoint if this is its only
						// connection.
        ...
        // ANCHOR MANAGER
        if (!params._transient) // in place copies, for example, are transient.
								// they will never need to be retrieved during a
								// paint cycle, because they dont move, and then
								// they are deleted.
        ...
        this.setEndpoint = function(ep) {
            ...
            // assign a clone function using a copy of endpointArgs. this is
			// used when a drag starts: the endpoint that was dragged is cloned,
            // and the clone is left in its place while the original one goes
			// off on a magical journey.
            // the copy is to get around a closure problem, in which
			// endpointArgs ends up getting shared by
            // the whole world.
        ...
        // bind listeners from endpoint to self, with the internal hover
        ...
        var _maxConnections = params.maxConnections || _jsPlumb.Defaults.MaxConnections; // maximum
																							// number
																							// of
																							// connections
																							// this
        ...
        // add anchor class (need to do this on construction because we set
		// anchor first)
        self.addClass(_jsPlumb.endpointAnchorClassPrefix + "_" + _currentAnchorClass);	
        jpcl.addClass(_element, _jsPlumb.endpointAnchorClassPrefix + "_" + _currentAnchorClass);
        ...
        this.detach = function(connection, ignoreTarget, forceDetach, fireEvent, originalEvent) {
            ...
            if (idx >= 0) {		
                // 1. does the connection have a before detach (note this also
				// checks jsPlumb's bound
                ...
                if (forceDetach || connection._forceDetach || connection.isDetachable() || connection.isDetachAllowed(connection)) {
                    ...
                    if (forceDetach || connection._forceDetach || (self.isDetachAllowed(connection) /*
                        ...
                        if (!ignoreTarget) {                        
                            ...
                            // check connection to see if we want to delete the
							// endpoints associated with it.
                            // we only detach those that have just this
							// connection; this scenario is most
                            // likely if we got to this bit of code because it
							// is set by the methods that
                            // create their own endpoints, like .connect or
							// .makeTarget. the user is
                            // not likely to have interacted with those
							// endpoints.
        ...
        this.setElement = function(el, container) {
            ...
            // remove the endpoint from the list for the current endpoint's
			// element
            ...
            // now move connection(s)...i would expect there to be only one but
			// we will iterate.
            for (var i = 0; i < self.connections.length; i++) {
        ...
        /**
		 * returns a connection from the pool; used when dragging starts. just
		 * gets the head of the array if it can.
		 */
        ...
        // a helper function that tries to find a connection to the given
		// element, and returns it if so. if elementWithPrecedence is null,
        // or no connection to it is found, we return the first connection in
		// our list.
        ...
        if (jpcl.isDragSupported(_element)) {
            ...
            var start = function() {	
            // drag might have started on an endpoint that is not actually a
			// source, but which has
            // one or more connections.
                ...
                // otherwise if we're full and not allowed to drag, also return
				// false.
                if (params.isSource && self.isFull() && !dragAllowedWhenFull) _continue = false;
                // if the connection was setup as not detachable or one of its
				// endpoints
                // was setup as connectionsDetachable = false, or
				// Defaults.ConnectionsDetachable
                // is set to false...
                if (jpc != null && !jpc.isDetachable()) _continue = false;

                if (_continue === false) {
                    // this is for mootools and yui. returning false from this
					// causes jquery to stop drag.
                    // the events are wrapped in both mootools and yui anyway,
					// but i don't think returning
                ...
                // if we're not full but there was a connection, make it null.
                ...
                // set the offset of this div to be where 'inPlaceCopy' is, to
				// start with.
                // TODO merge this code with the code in both Anchor and
				// FloatingAnchor, because it
                // does the same stuff.
                ...
                // when using makeSource and a parent, we first draw the source
				// anchor on the source element, then
                // move it to the parent. note that this happens after drawing
				// the placeholder for the
                // first time.
                if (self.parentAnchor) self.anchor = _jsPlumb.makeAnchor(self.parentAnchor, self.elementId, _jsPlumb);
                
                // store the id of the dragging div and the source element. the
				// drop function will pick these up.
                ...
                if (jpc == null) {                                                                                                                                                         
                    ...
                    // create a connection. one end is this endpoint, the other
					// is a floating endpoint.
                    jpc = _newConnection({
                        sourceEndpoint : self,
                        targetEndpoint : floatingEndpoint,
                        source : self.endpointWillMoveTo || _element,  // for
																		// makeSource
																		// with
																		// parent
																		// option.
                        ...
                        paintStyle : params.connectorStyle, // this can be null.
															// Connection will
															// use the default.
                        hoverPaintStyle:params.connectorHoverStyle,
                        connector : params.connector, // this can also be
														// null. Connection will
														// use the default.
                    ...
                    // fire an event that informs that a connection is being
					// dragged
                    ...
                    existingJpc = true;
                    jpc.setHover(false);						
                    // if existing connection, allow to be dropped back on the
					// source endpoint (issue 51).
                    _initDropTarget(ipcoel, false, true);
                    // new anchor idx
                    var anchorIdx = jpc.endpoints[0].id == self.id ? 0 : 1;
                    jpc.floatingAnchorIndex = anchorIdx;					// save
																			// our
																			// anchor
																			// index
																			// as
																			...
																			// floating
																			// index.
                    self.detachFromConnection(jpc);							// detach
																			// from
																			// the
																			// connection
																			// while
                    ...
                    // now we want to get this endpoint's DROP scope, and set it
					// for now: we can only be dropped on drop zones
                    // that have our drop scope (issue 57).
                    var dropScope = jpcl.getDropScope(canvasElement);
                    jpcl.setDragScope(canvasElement, dropScope);
            
                    // now we replace ourselves with the temporary div we
					// created above:
                    if (anchorIdx == 0) {
                    ...
                    // store the original endpoint and assign the new floating
					// endpoint for the drag.
                    jpc.suspendedEndpoint = jpc.endpoints[anchorIdx];
                    
                    // PROVIDE THE SUSPENDED ELEMENT, BE IT A SOURCE OR TARGET
					// (ISSUE 39)
            ...
            dragOptions[stopEvent] = _jsPlumb.wrap(dragOptions[stopEvent],
                ...
                // self.setPaintStyle(originalPaintStyle); // reset to original;
				// may have been changed by drag proxy.
                    ...
                    if (jpc.endpoints[idx] == floatingEndpoint) {
                        ...
                        if (existingJpc && jpc.suspendedEndpoint) {
                            ...
                            // TODO this looks suspiciously kind of like an
							// Endpoint.detach call too.
                            // i wonder if this one should post an event though.
                    ...
                    _ju.removeElements( [ placeholderInfo.element[0], floatingEndpoint.canvas ], _element); // TODO:
																											// clean
																											// up
																											// the
																											// connection
        ...
        // pulled this out into a function so we can reuse it for the
		// inPlaceCopy canvas; you can now drop detached connections
            ...
            if ((params.isTarget || forceInit) && jpcl.isDropSupported(_element)) {
                ...
                var dropEvent = jpcl.dragEvents['drop'],
                    ...
                    drop = function() {
                        ...
                        // if this is a drop back where the connection came
						// from, mark it force rettach and
                        // return; the stop handler will reattach. without
						// firing an event.
                        ...
                        if (jpc != null) {
                            ...
                            // restore the original scope if necessary (issue
							// 57)
                            ...
                            if (!self.isFull() && !(idx == 0 && !self.isSource) && !(idx == 1 && !self.isTarget) && endpointEnabled) {
                                ...
                                // the second check here is for the case that
								// the user is dropping it back
                                // where it came from.
                                ...
                                // these have to be set before testing for
								// beforeDrop.
                                if (idx == 0) {
...
// ------------ wrap the execution path in a function so we can support
// asynchronous beforeDrop
                                ...
                                var continueFunction = function() {
                                    ...
                                    // finalise will inform the anchor manager
									// and also add to
                                ...
                                var dontContinueFunction = function() {
                                    // otherwise just put it back on the
									// endpoint it was on before the drag.
                                    if (jpc.suspendedEndpoint) {									
                                        jpc.endpoints[idx] = jpc.suspendedEndpoint;
                                        jpc.setHover(false);
...
// --------------------------------------
                                // now check beforeDrop. this will be available
								// only on Endpoints that are setup to
                                // have a beforeDrop condition (although,
								// secretly, under the hood all Endpoints and
                                // the Connection have them, because they are on
								// jsPlumbUIComponent. shhh!), because
                                // it only makes sense to have it on a target
								// endpoint.
                ...
                dropOptions[overEvent] = _jsPlumb.wrap(dropOptions[overEvent], function() {					
                    ...
                    if (_jpc != null) {								
                        ...
                        // here we should fire the 'over' event if we are a
						// target and this is a new connection,
        ...
        // initialise the endpoint's canvas as a drop target. this will be
		// ignored if the endpoint is not a target or drag is not supported.
...
// ADD CLASS/REMOVE CLASS - override to support adding/removing to/from
// endpoints
...
// END HOVER
        ...
        this.setConnector = function(connectorSpec, doNotRepaint) {
            ...
            if (_ju.isString(connectorSpec)) 
                connector = makeConnector(renderMode, connectorSpec, connectorArgs); // lets
																						// you
																						// use
																						// a
																						// string
...
// INITIALISATION CODE
        ...
        // sourceEndpoint and targetEndpoint override source/target, if they are
		// present. but
        // source is not overridden if the Endpoint has declared it is not the
		// final target of a connection;
        // instead we use the source that the Endpoint declares will be the
        ...
        // if a new connection is the result of moving some existing connection,
		// params.previousConnection
        // will have that Connection in it. listeners for the jsPlumbConnection
		// event can look for that
        ...
        this.scope = params.scope; // scope may have been passed in to the
									// connect call. if it wasn't, we will pull
									// it from the source endpoint, after having
									// initialised the endpoints.
        this.endpoints = [];
        this.endpointStyles = [];
        // wrapped the main function to return null if no input given. this lets
		// us cascade defaults properly.
        ...
        prepareEndpoint = function(existing, index, params, element, elementId, connectorPaintStyle, connectorHoverPaintStyle) {
            ...
            if (existing) {
                ...
                // Endpoints derive their fillStyle from the connector's
				// strokeStyle, if no fillStyle was specified.
                if (es.fillStyle == null && connectorPaintStyle != null)
                    es.fillStyle = connectorPaintStyle.strokeStyle;
                
                // TODO: decide if the endpoint should derive the connection's
				// outline width and color. currently it does:
                // *
                ...
                // endpoint hover fill style is derived from connector's hover
				// stroke style. TODO: do we want to do this by default? for
				// sure?
        ...
        // if delete endpoints on detach, keep a record of just exactly which
		// endpoints they are.
        ...
        // TODO these could surely be refactored into some method that tries
		// them one at a time until something exists
...
// PARAMETERS
        ...
        // TODO jsPlumb.extend could be made to take more than two args, and it
		// would
...
// MISCELLANEOUS
        ...
        //
        // changes the parent element of this connection to newParent. not
		// exposed for the public API.
        //
        this.moveParent = function(newParent) {
            ...
            if (connector.reattachListeners)		// this is for SVG/VML;
													// change an element's
													// parent and you have to
													// reinit its listeners.
                connector.reattachListeners();     // the Canvas implementation
...
// PAINTING
		 ...
		 * 
		 * Parameters: elId - Id of the element that is in motion. ui - current
		 * library's event system ui object (present if we came from a drag to
		 * get here). recalc - whether or not to recalculate all anchors etc
		 * before painting. timestamp - timestamp of this paint. If the
		 * Connection was last painted with the same timestamp, it does not
		 * paint again.
		 */
        ...
        this.paint = function(params) {
            ...
            if (visible) {
                ...
                var elId = params.elId, ui = params.ui, recalc = params.recalc, timestamp = params.timestamp,
                    // if the moving object is not the source we must transpose
					// the two references.
                    swap = false,
                ...
                if (timestamp == null || timestamp != lastPaintedAt) {                        
                    var sourceInfo = _jsPlumb.updateOffset( { elId : elId, offset : ui, recalc : recalc, timestamp : timestamp }).o,
                        targetInfo = _jsPlumb.updateOffset( { elId : tId, timestamp : timestamp }).o, // update
																										// the
																										// target
																										// if
																										// this
                    ...
                    // compute overlays. we do this first so we can get their
					// placements, and adjust the
        ...
        /*
		 * Function: repaint Repaints the Connection. No parameters exposed to
		 * public API.
		 */
        ...
        // the very last thing we do is check to see if a 'type' was supplied in
		// the params
...
// END PAINTING
		 ...
		 * either SVG, Canvas elements, or VML.
		 * 
		 * This file contains the default Connectors, Endpoint and Overlay
		 * definitions.
		 * 
		 * Copyright (c) 2010 - 2013 Simon Porritt (http://jsplumb.org)
		 * 
...
;(function() {	
	...
	/**
	 * 
	 * Helper class to consume unused mouse events by components that are DOM
	 * elements and are used by all of the different rendering modes.
	 * 
	 */
	jsPlumb.DOMElementComponent = function(params) {
		jsPlumb.jsPlumbUIComponent.apply(this, arguments);
		// when render mode is canvas, these functions may be called by the
	...
	jsPlumb.Segments = {
        ...
        /*
		 * Class: AbstractSegment A Connector is made up of 1..N Segments, each
		 * of which has a Type, such as 'Straight', 'Arc', 'Bezier'. This is new
		 * from 1.4.1, and gives us a lot more flexibility when drawing
		 * connections: things such as rounded corners for flowchart connectors,
		 * for example, or a straight line stub for Bezier connections, are much
		 * easier to do now.
		 * 
		 * A Segment is responsible for providing coordinates for painting it,
		 * and also must be able to report its length.
		 * 
		 */ 
        AbstractSegment : function(params) { 
            ...
            /**
			 * Function: findClosestPointOnPath Finds the closest point on this
			 * segment to the given [x, y], returning both the x and y of the
			 * point plus its distance from the supplied point, and its location
			 * along the length of the path inscribed by the segment. This
			 * implementation returns Infinity for distance and null values for
			 * everything else; subclasses are expected to override.
			 */
        ...
        Straight : function(params) {
            ...
            /**
			 * returns the point on the segment's path that is 'location' along
			 * the length of the path, where 'location' is a decimal from 0 to 1
			 * inclusive. for the straight line segment this is simple maths.
			 */
            ...
            /**
			 * returns the gradient of the segment at the given point - which
			 * for us is constant.
			 */
            ...
            /**
			 * returns the point on the segment's path that is 'distance' along
			 * the length of the path from 'location', where 'location' is a
			 * decimal from 0 to 1 inclusive, and 'distance' is a number of
			 * pixels. this hands off to jsPlumbUtil to do the maths, supplying
            ...
            /**
			 * Function: findClosestPointOnPath Finds the closest point on this
			 * segment to [x,y]. See notes on this method in AbstractSegment.
			 */
            this.findClosestPointOnPath = function(x, y) {
                if (m == 0) {
                    return {
                    ...
                    // closest point lies on normal from given point to this
					// line.
                    var b = y1 - (m * x1),
                        b2 = y - (m2 * x),                    
		 ...
		 * 
		 * r - radius cx - center x for the arc cy - center y for the arc ac -
		 * whether the arc is anticlockwise or not. default is clockwise.
		 * 
		 * and then either:
		 * 
		 * startAngle - startAngle for the arc. endAngle - endAngle for the arc.
		 * 
		 * or:
		 * 
		 * x1 - x for start point y1 - y for start point x2 - x for end point y2 -
		 * y for end point
		 * 
		 */
        Arc : function(params) {
            ...
            // we now have startAngle and endAngle as positive numbers, meaning
			// the
            // absolute difference (|d|) between them is the sweep (s) of this
			// arc, unless the
        ...
        Bezier : function(params) {
            var self = this,
                ...
                // although this is not a strictly rigorous determination of
				// bounds
                // of a bezier curve, it works for the types of curves that this
				// segment
                // type produces.
                bounds = {
    ...
    /*
	 * Class: AbstractComponent Superclass for AbstractConnector and
	 * AbstractEndpoint.
	 */
	...
	/*
	 * Class: AbstractConnector Superclass for all Connectors; here is where
	 * Segments are managed. This is exposed on jsPlumb just so it can be
	 * accessed from other files. You should not try to instantiate one of these
	 * directly.
	 * 
	 * When this class is asked for a pointOnPath, or gradient etc, it must
	 * first figure out which segment to dispatch that request to. This is done
	 * by keeping track of the total connector length as segments are added, and
	 * also their cumulative ratios to the total length. Then when the right
	 * segment is found it is a simple case of dispatching the request to it
	 * (and adjusting 'location' so that it is relative to the beginning of that
	 * segment.)
	 */ 
	jsPlumb.Connectors.AbstractConnector = function(params) {
        ...
        /**
		 * Function: findSegmentForPoint Returns the segment that is closest to
		 * the given [x,y], null if nothing found. This function returns a JS
		 * object with:
		 * 
		 * d - distance from segment l - proportional location in segment x - x
		 * point on the segment y - y point on the segment s - the segment
		 * itself.
		 */ 
		...
		var _updateSegmentProportions = function() {
            ...
            /**
			 * returns [segment, proportion of travel in segment, segment index]
			 * for the segment that contains the point which is 'location'
			 * distance along the entire path, where 'location' is a decimal
			 * between 0 and 1 inclusive. in this connector type, paths are made
			 * up of a list of segments, each of which contributes some fraction
			 * to the total length. From 1.3.10 this also supports the
			 * 'absolute' property, which lets us specify a location as the
			 * absolute distance in pixels, rather than a proportion of the
			 * total path.
			 */
                ...
                // if (location < 1) {
                    for (var i = 0; i < segmentProportions.length; i++) {
                        if (segmentProportions[i][1] >= location) {
                            idx = i;
                            // todo is this correct for all connector path
							// types?
        ...
        var _prepareCompute = function(params) {
            ...
            // if either anchor does not have an orientation set, we derive one
			// from their relative
            // positions. we fix the axis to be the one in which the two
			// elements are further apart, and
            // point each anchor at the other element. this is also used when
    ...
    /**
	 * Class: Connectors.Straight The Straight connector draws a simple straight
	 * line between the two anchor points. It does not have any constructor
	 * parameters.
	 */
    ...
    /**
	 * Class:Connectors.Bezier This Connector draws a Bezier curve with two
	 * control points. You can provide a 'curviness' value which gets applied to
	 * jsPlumb's internal voodoo machine and ends up generating locations for
	 * the two control points. See the constructor documentation below.
	 */
    /**
	 * Function:Constructor
	 * 
	 * Parameters: curviness - How 'curvy' you want the curve to be! This is a
	 * directive for the placement of control points, not endpoints of the
	 * curve, so your curve does not actually touch the given point, but it has
	 * the tendency to lean towards it. The larger this value, the greater the
	 * curve is pulled from a straight line. Optional; defaults to 150. stub -
	 * optional value for a distance to travel from the connector's endpoint
	 * before beginning the Bezier curve. defaults to 0.
	 * 
	 */
    jsPlumb.Connectors.Bezier = function(params) {
        ...
        this._findControlPoint = function(point, sourceAnchorPosition, targetAnchorPosition, sourceEndpoint, targetEndpoint) {
        	// determine if the two anchors are perpendicular to each other in
			// their orientation. we swap the control
 ...
 // ********************************* ENDPOINT TYPES
    ...
    /**
	 * Class: Endpoints.Dot A round endpoint, with default radius 10 pixels.
	 */    	
	...
	/**
	 * Class: Endpoints.Rectangle A Rectangular Endpoint, with default size
	 * 20x20.
	 */
	/**
	 * Function: Constructor
	 ...
	 * Parameters:
	 * 
	 * width - width of the endpoint. defaults to 20 pixels. height - height of
	 * the endpoint. defaults to 20 pixels.
	 */
	...
	/**
	 * Class: Endpoints.Image Draws an image as the Endpoint.
	 */
	/**
	 * Function: Constructor
	 * 
	...
	jsPlumb.Endpoints.Image = function(params) {
        ...
        /*
		 * Function: setImage Sets the Image to use in this Endpoint.
		 * 
		 * Parameters: img - may be a URL or an Image object onload - optional;
		 * a callback to execute once the image has loaded.
		 */
	...
	/*
	 * Class: Endpoints.Blank An Endpoint that paints nothing (visible) on the
	 * screen. Supports cssClass and hoverClass parameters like all Endpoints.
	 */
	...
	/*
	 * Class: Endpoints.Triangle A triangular Endpoint.
	 */
	/*
	 * Function: Constructor
	 * 
	 * Parameters:
	 * 
	 * width - width of the triangle's base. defaults to 55 pixels. height -
	 * height of the triangle from base to apex. defaults to 55 pixels.
	 */
...
// ***********************************************************************
	 ...
	 * 
	 * An arrow overlay, defined by four points: the head, the two sides of the
	 * tail, and a 'foldback' point at some distance along the length of the
	 * arrow that lines from each tail point converge into. The foldback point
	 * is defined using a decimal that indicates some fraction of the length of
	 * the arrow and has a default value of 0.623. A foldback point value of 1
	 * would mean that the arrow had a straight line across the tail.
	 */
	/*
	 * Function: Constructor
	 * 
	 * Parameters:
	 * 
	 * length - distance in pixels from head to tail baseline. default 20. width -
	 * width in pixels of the tail baseline. default 20. fillStyle - style to
	 * use when filling the arrow. defaults to "black". strokeStyle - style to
	 * use when stroking the arrow. defaults to null, which means the arrow is
	 * not stroked. lineWidth - line width to use when stroking the arrow.
	 * defaults to 1, but only used if strokeStyle is not null. foldback -
	 * distance (as a decimal from 0 to 1 inclusive) along the length of the
	 * arrow marking the point the tail points should fold back to. defaults to
	 * 0.623. location - distance (as a decimal from 0 to 1 inclusive) marking
	 * where the arrow should sit on the connector. defaults to 0.5. direction -
	 * indicates the direction the arrow points in. valid values are -1 and 1; 1
	 * is default.
	 */
	jsPlumb.Overlays.Arrow = function(params) {
    	...
    	var direction = (params.direction || 1) < 0 ? -1 : 1,
    	    ...
    	    // how far along the arrow the lines folding back in come to.
			// default is 62.3%.
	 ...
	 * 
	 * A basic arrow. This is in fact just one instance of the more generic case
	 * in which the tail folds back on itself to some point along the length of
	 * the arrow: in this case, that foldback point is the full length of the
	 * arrow. so it just does a 'call' to Arrow with foldback set appropriately.
	 */
    /*
	 * Function: Constructor See <Overlays.Arrow> for allowed parameters for
	 * this overlay.
	 */
	 ...
	 * 
	 * A diamond. Like PlainArrow, this is a concrete case of the more generic
	 * case of the tail points converging on some point...it just happens that
	 * in this case, that point is greater than the length of the the arrow.
	 * 
	 * this could probably do with some help with positioning...due to the way
	 * it reuses the Arrow paint code, what Arrow thinks is the center is
	 * actually 1/4 of the way along for this guy. but we don't have any
	 * knowledge of pixels at this point, so we're kind of stuck when it comes
	 * to helping out the Arrow class. possibly we could pass in a 'transpose'
	 * parameter or something. the value would be -l/4 in this case - move along
    ...
    var AbstractDOMOverlay = function(params) {
		...
		/*
		 * Function: clearCachedDimensions Clears the cached dimensions for the
		 * label. As a performance enhancement, label dimensions are cached from
		 * 1.3.12 onwards. The cache is cleared when you change the label text,
		 * of course, but there are other reasons why the text dimensions might
		 * change - if you make a change through CSS, for example, you might
		 * change the font size. in that case you should explicitly call this
		 * method.
		 */
		...
		this.draw = function(component, currentConnectionPaintStyle) {
	    	...
	    	if (td != null && td.length == 2) {
                ...
                if (component.pointOnPath) {
                    ...
                    cxy = component.pointOnPath(loc, absolute);  // a
																	// connection
                }
                else {
	...
	/*
	 * Class: Overlays.Custom A Custom overlay. You supply a 'create' function
	 * which returns some DOM element, and jsPlumb positions it. The 'create'
	 * function is passed a Connection or Endpoint.
	 */
    /*
	 * Function: Constructor
	 * 
	 * Parameters: create - function for jsPlumb to call that returns a DOM
	 * element. location - distance (as a decimal from 0 to 1 inclusive) marking
	 * where the label should sit on the connector. defaults to 0.5. id -
	 * optional id to use for later retrieval of this overlay.
	 * 
	 */
    ...
    /*
	 * Class: Overlays.Label A Label overlay. For all different renderer types
	 * (SVG/Canvas/VML), jsPlumb draws a Label overlay as a styled DIV. Version
	 * 1.3.0 of jsPlumb introduced the ability to set css classes on the label;
	 * this is now the preferred way for you to style a label. The 'labelStyle'
	 * parameter is still supported in 1.3.0 but its usage is deprecated. Under
	 * the hood, jsPlumb just turns that object into a bunch of CSS directive
	 * that it puts on the Label's 'style' attribute, so the end result is the
	 * same.
	 */
    /*
	 * Function: Constructor
	 * 
	 * Parameters: cssClass - optional css class string to append to css class.
	 * This string is appended "as-is", so you can of course have multiple
	 * classes defined. This parameter is preferred to using labelStyle,
	 * borderWidth and borderStyle. label - the label to paint. May be a string
	 * or a function that returns a string. Nothing will be painted if your
	 * label is null or your label function returns null. empty strings _will_
	 * be painted. location - distance (as a decimal from 0 to 1 inclusive)
	 * marking where the label should sit on the connector. defaults to 0.5. id -
	 * optional id to use for later retrieval of this overlay.
	 * 
	 */
    jsPlumb.Overlays.Label = function(params) {
    	...
    	/*
		 * Function: setLabel sets the label's, um, label. you would think i'd
		 * call this function 'setText', but you can pass either a Function or a
		 * String to this, so it makes more sense as 'setLabel'. This uses
		 * innerHTML on the label div, so keep that in mind if you need escaped
		 * HTML.
		 */
    	this.setLabel = function(l) {
    		label = l;
 ...
 // ********************************* END OF OVERLAY DEFINITIONS
	// ***********************************************************************
		 ...
		 * 
		 * Thanks to Brainstorm Mobile Solutions for supporting the development
		 * of these.
		 * 
		 * Copyright (c) 2010 - 2013 Simon Porritt (simon.porritt@gmail.com)
		 * 
...
;(function() {
	...
	var Line = function(x1, y1, x2, y2) {
		...
		this.rectIntersect = function(x,y,w,h) {
			...
			if (results.length == 2) {
				...
				// now calculate the segment inside the rectangle where the
				// midpoint lies.
	...
	_segment = function(x1, y1, x2, y2) {
		...
		// the control point we will use depends on the faces to which each end
		// of the connection is assigned, specifically whether or not the
		// two faces are parallel or perpendicular. if they are parallel then
		// the control point lies on the midpoint of the axis in which they
		// are parellel and varies only in the other axis; this variation is
		// proportional to the distance that the anchor points lie from the
		// center of that face. if the two faces are perpendicular then the
		// control point is at some distance from both the midpoints; the amount
		// and
		// direction are dependent on the orientation of the two elements.
		// 'seg', passed in to this method, tells you which segment the target
		// element
		// lies in with respect to the source: 1 is top right, 2 is bottom
		// right, 3 is bottom left, 4 is top left.
		//
		// sourcePos and targetPos are arrays of info about where on the source
		// and target each anchor is located. their contents are:
		//
		// 0 - absolute x
		// 1 - absolute y
	...
	_findControlPoint = function(midx, midy, segment, sourceEdge, targetEdge, dx, dy, distance, proximityLimit) {
        // TODO (maybe)
        // - if anchor pos is 0.5, make the control point take into account the
	...
	/**
	 * Class: Connectors.StateMachine Provides 'state machine' connectors.
	 */
	/*
	 * Function: Constructor
	 * 
	 * Parameters: curviness - measure of how "curvy" the connectors will be.
	 * this is translated as the distance that the Bezier curve's control point
	 * is from the midpoint of the straight line connecting the two endpoints,
	 * and does not mean that the connector is this wide. The Bezier curve never
	 * reaches its control points; they act as gravitational masses. defaults to
	 * 10. margin - distance from element to start and end connectors, in
	 * pixels. defaults to 5. proximityLimit - sets the distance beneath which
	 * the elements are consider too close together to bother with fancy curves.
	 * by default this is 80 pixels. loopbackRadius - the radius of a loopback
	 * connector. optional; defaults to 25. showLoopback - If set to false this
	 * tells the connector that it is ok to paint connections whose source and
	 * target is the same element with a connector running through the element.
	 * The default value for this is true; the connector always makes a loopback
	 * connection loop around the element rather than passing through it.
	 */
	jsPlumb.Connectors.StateMachine = function(params) {
		...
		this._compute = function(paintInfo, params) {
			...
			if (!showLoopback || (params.sourceEndpoint.elementId !== params.targetEndpoint.elementId)) {                            
            	...
            	//
	            // these connectors are quadratic bezier curves, having a single
				// control point. if both anchors
    	        // are located at 0.5 on their respective faces, the control
				// point is set to the midpoint and you
        	    // get a straight line. this is also the case if the two anchors
				// are within 'proximityLimit', since
           	 	// it seems to make good aesthetic sense to do that. outside of
				// that, the control point is positioned
           	 	// at 'curviness' pixels away along the normal to the straight
				// line connecting the two anchors.
	            // 
   	        	// there may be two improvements to this. firstly, we might
				// actually support the notion of avoiding nodes
            	// in the UI, or at least making a good effort at doing so. if a
				// connection would pass underneath some node,
            	// for example, we might increase the distance the control point
				// is away from the midpoint in a bid to
            	// steer it around that node. this will work within limits, but
				// i think those limits would also be the likely
            	// limits for, once again, aesthetic good sense in the layout of
				// a chart using these connectors.
            	//
            	// the second possible change is actually two possible changes:
				// firstly, it is possible we should gradually
            	// decrease the 'curviness' as the distance between the anchors
				// decreases; start tailing it off to 0 at some
            	// point (which should be configurable). secondly, we might
				// slightly increase the 'curviness' for connectors
            	// with respect to how far their anchor is from the center of
				// its respective face. this could either look cool,
            	// or stupid, and may indeed work only in a way that is so
				// subtle as to have been a waste of time.
            	//
				...
				var _midx = (_sx + _tx) / 2, _midy = (_sy + _ty) / 2, 
	            	...
	            	// calculate the control point. this code will be where
					// we'll put in a rudimentary element avoidance scheme; it
	            	// will work by extending the control point to force the
					// curve to be, um, curvier.
            	...
            	// a loopback connector. draw an arc from one anchor to the
				// other.
        		var x1 = params.sourcePos[0], x2 = params.sourcePos[0], y1 = params.sourcePos[1] - margin, y2 = params.sourcePos[1] - margin, 				
					cx = x1, cy = y1 - loopbackRadius;
				
					// canvas sizing stuff, to ensure the whole painted area is
					// visible.
					w = 2 * loopbackRadius, 
					h = 2 * loopbackRadius,
					x = cx - loopbackRadius, 
 ...
 * if (avoidSelector) { // var testLine = new Line(sourcePos[0] +
 * _sx,sourcePos[1] + _sy,sourcePos[0] + _tx,sourcePos[1] + _ty); // var sel =
 * jsPlumb.getSelector(avoidSelector); // for (var i = 0; i < sel.length; i++) { //
 * var id = jsPlumb.getId(sel[i]); // if (id != sourceEndpoint.elementId && id !=
 * targetEndpoint.elementId) { // o = jsPlumb.getOffset(id), s =
 * jsPlumb.getSize(id); // // if (o && s) { // var collision =
 * testLine.rectIntersect(o.left,o.top,s[0],s[1]); // if (collision) { // set
 * the control point to be a certain distance from the midpoint of the two
 * points that // the line crosses on the rectangle. // TODO where will this 75
 * number come from? // _controlX = collision[2][0] + (75 * collision[3][0]); // /
 ...
 *//*
	 ...
	 * SVG, Canvas elements, or VML.
	 * 
	 * This file contains the 'flowchart' connectors, consisting of vertical and
...
;(function() {
	 ...
	 * Function: Constructor
	 * 
	 * Parameters: stub - minimum length for the stub at each end of the
	 * connector. This can be an integer, giving a value for both ends of the
	 * connections, or an array of two integers, giving separate values for each
	 * end. The default is an integer with value 30 (pixels). gap - gap to leave
	 * between the end of the connector and the element on which the endpoint
	 * resides. if you make this larger than stub then you will see some odd
	 * looking behaviour. Like stub, this can be an array or a single value.
	 * defaults to 0 pixels for each end. cornerRadius - optional, defines the
	 * radius of corners between segments. defaults to 0 (hard edged corners).
	 * alwaysRespectStubs - defaults to false. whether or not the connectors
	 * should always draw the stub, or, if the two elements are in close
	 * proximity to each other (closer than the sum of the two stubs), to adjust
	 * the stubs.
	 */
    jsPlumb.Connectors.Flowchart = function(params) {
            ...
            writeSegments = function(segments, paintInfo) {
                ...
                for (var i = 0; i < segments.length - 1; i++) {
                    ...
                    if (cornerRadius > 0 && current[4] != next[4]) {
                        ...
                        // right angle. adjust current segment's end point, and
						// next segment's start point.
        ...
        /*
		 * Function: getOriginalSegments Gets the segments before the addition
		 * of rounded corners. This is used by the flowchart connector editor,
		 * since it only wants to concern itself with the original segments.
		 */
...
;(function() {
	...
	if (document.createStyleSheet && document.namespaces) {			
		...
		// in this page it is also mentioned that IE requires the extra arg to
		// the namespace
		// http://www.louisremi.com/2009/03/30/changes-in-vml-for-ie8-or-what-feature-can-the-ie-dev-team-break-for-you-today/
		// but someone commented saying they didn't need it, and it seems
		// jsPlumb doesnt need it either.
    ...
    _getGroup = function(container, connectorClass) {
        ...
        if(!g) {
            ...
            _groupMap[id] = g;
            jsPlumb.appendElement(g, container);  // todo if this gets
													// reinstated, remember to
													// use the current jsplumb
													// instance.
	...
	_atts = function(o, atts) {
		for (var i in atts) { 
			// IE8 fix: setattribute does not work after an element has been
			// added to the dom!
			// http://www.louisremi.com/2009/03/30/changes-in-vml-for-ie8-or-what-feature-can-the-ie-dev-team-break-for-you-today/
			// o.setAttribute(i, atts[i]);

			/*
			 * There is an additional problem when accessing VML elements by
			 * using get/setAttribute. The simple solution is following:
			 * 
			 * if (document.documentMode==8) { ele.opacity=1; } else {
	...
	// tests if the given style is "transparent" and then sets the appropriate
	// opacity node to 0 if so,
		...
		this.paint = function(style) {		
			if (style !== null) {				
				...
				if (self.canvas == null) {										
                    ...
                    // var group = _getGroup(params.parent); // test of append
					// everything to a group
                    // group.appendChild(self.canvas); // sort of works but not
					// exactly;
					// params["_jsPlumb"].appendElement(self.canvas,
					// params.parent); //before introduction of groups
	...
	VmlEndpoint = window.VmlEndpoint = function(params) {
		...
		// TODO vml endpoint adds class to VML at constructor time. but the
		// addClass method adds VML
		// to the enclosing DIV. what to do? seems like it would be better to
		// just target the div.
		// HOWEVER...vml connection has no containing div. why not? it feels
		// like it should.
...
// *****************************************************
	...
	/**
	 * VML Label renderer. uses the default label renderer (which adds an
	 * element to the DOM)
	 */
	jsPlumb.Overlays.vml.Label  = jsPlumb.Overlays.Label;
	
	/**
	 * VML Custom renderer. uses the default Custom renderer (which adds an
	 * element to the DOM)
	 */
	...
	var AbstractVmlArrowOverlay = function(superclass, originalArgs) {
    	...
    	this.paint = function(params, containerExtents) {
			...
			// for VML, we create overlays using shapes that have the same
			// dimensions and
			// coordsize as their connector - overlays calculate themselves
			// relative to the
			// connector (it's how it's been done since the original canvas
 ...
 * css:http://tutorials.jenkov.com/svg/svg-and-css.html text on a path:
 * http://www.w3.org/TR/SVG/text.html#TextOnAPath pointer events:
...
// ********************************************
	...
	_updateGradient = function(parent, node, style, dimensions, uiComponent) {
		...
		// this checks for an 'offset' property in the gradient, and in the
		// absence of it, assumes
		// we want a linear gradient. if it's there, we create a radial
		// gradient.
		// it is possible that a more explicit means of defining the gradient
		// type would be
		// better. relying on 'offset' means that we can never have a radial
		// gradient that uses
		// some default offset, for instance.
		// issue 244 suggested the 'gradientUnits' attribute; without this,
		// straight/flowchart connectors with gradients would
		// not show gradients when the line was perfectly horizontal or
		// vertical.
		var g;
		...
		// order to how canvas does it. so we want to keep all the maths the
		// same, but
		// iterate the actual style declarations in reverse order, if the x
	...
	_applyStyles = function(parent, node, style, dimensions, uiComponent) {
		...
		// in SVG there is a stroke-dasharray attribute we can set, and its
		// syntax looks like
		// the syntax in VML but is actually kind of nasty: values are given in
		// the pixel
		// coordinate space, whereas in VML they are multiples of the width of
		// the stroked
		// line, which makes a lot more sense. for that reason, jsPlumb is
		// supporting both
		// the native svg 'stroke-dasharray' attribute, and also the 'dashstyle'
		// concept from
		// VML, which will be the preferred method. the code below this converts
		// a dashstyle
		// attribute given in terms of stroke width into a pixel representation,
		// by using the
		// stroke's lineWidth.
 ...
 // ************************** / SVG utility methods
	...
	var SvgComponent = function(params) {
		...
		// TODO this displayElement stuff is common between all components,
		// across all
	...
	var SvgConnector = jsPlumb.ConnectorRenderers.svg = function(params) {
		...
		_super.renderer.paint = function(style, anchor, extents) {
			...
			// outline style. actually means drawing an svg object underneath
			// the main one.
			if (style.outlineColor) {
				var outlineWidth = style.outlineWidth || 1,
				outlineStrokeWidth = style.lineWidth + (2 * outlineWidth),
...
// *****************************************************
    ...
    jsPlumb.Overlays.svg.GuideLines = function() {
		 ...
		 */ 
/*
 * the library specific functions, such as find offset, get id, get attribute,
 * extend etc. the full list is:
 * 
 * addClass adds a class to the given element animate calls the underlying
 * library's animate functionality appendElement appends a child element to a
 * parent element. bind binds some event to an element dragEvents a dictionary
 * of event names extend extend some js object with another. probably not overly
 * necessary; jsPlumb could just do this internally. getAttribute gets some
 * attribute from an element getDragObject gets the object that is being
 * dragged, by extracting it from the arguments passed to a drag callback
 * getDragScope gets the drag scope for a given element. getDropScope gets the
 * drop scope for a given element. getElementObject turns an id or dom element
 * into an element object of the underlying library's type. getOffset gets an
 * element's offset getOriginalEvent gets the original browser event from some
 * wrapper event getPageXY gets the page event's xy location. getParent gets the
 * parent of some element. getScrollLeft gets an element's scroll left. TODO: is
 * this actually used? will it be? getScrollTop gets an element's scroll top.
 * TODO: is this actually used? will it be? getSize gets an element's size.
 * getUIPosition gets the position of some element that is currently being
 * dragged, by extracting it from the arguments passed to a drag callback.
 * hasClass returns whether or not the given element has the given class.
 * initDraggable initializes an element to be draggable initDroppable
 * initializes an element to be droppable isDragSupported returns whether or not
 * drag is supported for some element. isDropSupported returns whether or not
 * drop is supported for some element. removeClass removes a class from a given
 * element. removeElement removes some element completely from the DOM.
 * setAttribute sets an attribute on some element. setDragFilter sets a filter
 * for some element that indicates areas of the element that should not respond
 * to dragging. setDraggable sets whether or not some element should be
 * draggable. setDragScope sets the drag scope for a given element. setOffset
 * sets the offset of some element. trigger triggers some event on an element.
 * unbind unbinds some listener from some element.
 */
(function($) {	
	
	// var getBoundingClientRectSupported = "getBoundingClientRect" in
	...
	jsPlumb.CurrentLibrary = {					        
		...
		/**
		 * event binding wrapper. it just so happens that jQuery uses 'bind'
		 * also. yui3, for example, uses 'on'.
		 */
		...
		/**
		 * wrapper around the library's 'extend' functionality (which it
		 * hopefully has. otherwise you'll have to do it yourself). perhaps
		 * jsPlumb could do this for you instead. it's not like its hard.
		 */
		...
		/**
		 * takes the args passed to an event function and returns you an object
		 * representing that which is being dragged.
		 */
		...
		/**
		 * gets a DOM element from the given input, which might be a string (in
		 * which case we just do document.getElementById), a selector (in which
		 * case we return el[0]), or a DOM element already (we assume this if
		 * it's not either of the other two cases). this is the opposite of
		 * getElementObject below.
		 */
		...
		/**
		 * gets an "element object" from the given input. this means an object
		 * that is used by the underlying library on which jsPlumb is running.
		 * 'el' may already be one of these objects, in which case it is
		 * returned as-is. otherwise, 'el' is a String, the library's lookup
		 * function is used to find the element, using the given String as the
		 * element's id.
		 * 
		 */		
		...
		/**
		 * gets the offset for the element object. this should return a js
		 * object like this: { left:xxx, top: xxx }
		 */
		...
		/**
		 * takes the args passed to an event function and returns you an object
		 * that gives the position of the object being moved, as a js object
		 * with the same params as the result of getOffset, ie: { left: xxx,
		 * top: xxx }.
		 * 
		 * different libraries have different signatures for their event
		 * callbacks. see getDragObject as well
		 */
		getUIPosition : function(eventArgs, zoom) {
			
			zoom = zoom || 1;
			// this code is a workaround for the case that the element being
			// dragged has a margin set on it. jquery UI passes
			// in the wrong offset if the element has a margin (it doesn't take
			// the margin into account). the getBoundingClientRect
			// method, which is in pretty much all browsers now, reports the
			// right numbers. but it introduces a noticeable lag, which
			// i don't like.
            
			/*
			 * if ( getBoundingClientRectSupported ) { var r =
			 * eventArgs[1].helper[0].getBoundingClientRect(); return { left :
			 * r.left, top: r.top }; } else {
			 */
			if (eventArgs.length == 1) {
				...
				// adjust ui position to account for zoom, because jquery ui
				// does not do this.
 ...
 * options.start = _jsPlumb.wrap(options["start"], function(e, ui) { // TODO why
 * is this 0? ui.position.left = 0; ui.position.top = 0; });
 * 
 ...
 * 
 * //var changeLeft = ui.position.left - ui.originalPosition.left; // find
 * change in left //var newLeft = ui.originalPosition.left + (changeLeft *
 * _jsPlumb.getZoom()); // adjust new left by our zoomScale
 * 
 * //var changeTop = ui.position.top - ui.originalPosition.top; // find change
 * in top //var newTop = ui.originalPosition.top + (changeTop *
 * _jsPlumb.getZoom()); // adjust new top by our zoomScale
 * 
 * //ui.position.left = newLeft; //ui.position.top = newTop;
 * 
 * ui.position.left *= _jsPlumb.getZoom(); ui.position.top *=
 * _jsPlumb.getZoom();
 * 
 * });
 */
		...
		/**
		 * returns whether or not drag is supported (by the library, not whether
		 * or not it is disabled) for the given element.
		 */
		...
		/**
		 * returns whether or not drop is supported (by the library, not whether
		 * or not it is disabled) for the given element.
		 */
		...
		/**
		 * note that jquery ignores the name of the event you wanted to trigger,
		 * and figures it out for itself. the other libraries do not. yui, in
		 * fact, cannot even pass an original event. we have to pull out stuff
